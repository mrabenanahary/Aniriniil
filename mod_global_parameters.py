...
def create_box_file_dict_name(xmin,xmax,ymin,ymax):
    return  "{:.2e}".format(xmin)+'_infEq_x_infEq_'+\
            "{:.2e}".format(xmax)+','+\
            "{:.2e}".format(ymin)+'_infEq_y_infEq_'+\
            "{:.2e}".format(ymax)
...
class Variable:
    ...
    input_file_vtk = "./Jet_CC_Shang06_0296.vtu"#"./Jet_Shang06_B4_0090_LEVEL5.vtu"#"./input/Jet_cylindric_pulser_Lee2001/hllc/old/Jet_CC_Shang06_0610.vtu"
    output_file_vtk = "./Cylindric_jet_nG237_nP60_296_ans"#"./Cylindric_jet_nG237_nP60_10000_ans_i120deg"#"./hllc/DE/Lee_2001_us_300yrs_unstratified/Cylindric_jet_nG237_nP60_296_ans"
    output_file_vtk_mv = "./Cylindric_jet_nG237_nP60_296_ans_mv"#"./Cylindric_jet_nG237_nP60_10000_ans_i120deg"#"./hllc/DE/Lee_2001_us_300yrs_unstratified/Cylindric_jet_nG237_nP60_296_ans"
    output_mv_spectrum = True
    ...
    plotting = True
    plotting_box = True
    variable_to_plot = ['number density','v1','v2']
    variable_label = [r'$\mathrm{\rho}$ ($\mathrm{g~cm^{-3}}$)',
                      r'$\mathrm{v_R}$ ($\mathrm{km~s^{-1}}$)',
                      r'$\mathrm{v_Z}$ ($\mathrm{km~s^{-1}}$)']
    
    plot_title = [r'\textbf{Denser flattened singular core (PCJ-RW)}',
                  r'\textbf{Denser flattened singular core (PCJ-RW)}',
                  r'\textbf{Denser flattened singular core (PCJ-RW)}'] 
    plot_title_pad = [30.0,30.0,30.0]
    plot_height = [8.0,8.0,8.0]
    plot_width = [8.0,8.0,8.0]
    plot_xmin=[0,0,0]
    plot_xmax=[0.6,0.6,0.6]
    plot_dr = [0.2,0.2,0.2]
    plot_ymin=[0,0,0]
    plot_ymax=[1.35,1.35,1.35]
    plot_vmin=[3e-2,0,0]
    plot_vmax=[1.0e7,50,50]
    plot_fontsize = [40,40,40]
    plot_fontsize_title = [40,40,40]
    plot_fontsize_cbr = [30,30,30]
    plot_fontsize_global = [25,25,25]
    plot_colormap = ['my_cmap','turbo','turbo']
    plot_darken_factor = [0.9,0.9,0.9]
    plot_rect_lw = [4,4]
    plot_use_sns = [True,True,True,True]
    plot_sns_palette = ['tab10','tab10','tab10']
    plot_mpl_palette = ['tab20','tab20','tab20']
    plot_sns_palette_n = [20,20,20]
    plot_mpl_palette_n = [10,10,10]
    
    #include_frame_box = True
    x_frame = r_
    y_frame = z_cyl_
    x_frame_min = [0.0,0.0]#2.5e15#0.0
    x_frame_max = [1.0e16,2.0e16]#3.5e15#1.2e17
    y_frame_min = [0.0,0.0]
    y_frame_max = [1.45e17,1.45e17]
...
class Bazooka:
    ...
    if(Variable.frame_the_box):
        geo_position_f_box_inc = {}
        geo_position_f_box_exc = {}
        geo_position_cellsPoints_masse_box_inc={}
        geo_position_cellsPoints_masse_box_exc={}
        for box_i,box_el in enumerate(Variable.x_frame_min):
            my_dict_name = create_box_file_dict_name(Variable.x_frame_min[box_i],
                                                     Variable.x_frame_max[box_i],
                                                     Variable.y_frame_min[box_i],
                                                     Variable.y_frame_max[box_i])
            geo_position_f_box_inc[my_dict_name] = np.empty((numberOf3Delem,))
            geo_position_f_box_exc[my_dict_name] = np.empty((numberOf3Delem,))
        #geo_position_masse_box_inc = np.empty((numberOf3Delem,))
        #geo_position_masse_box_exc = np.empty((numberOf3Delem,))
            geo_position_cellsPoints_masse_box_inc[my_dict_name] = np.empty((numberOf3Delem,))
            geo_position_cellsPoints_masse_box_exc[my_dict_name] = np.empty((numberOf3Delem,))        
        if(Variable.frame_ism):
            geo_position_f_box_ism_inc = {}
            geo_position_f_box_ism_exc = {}
            geo_position_cellsPoints_masse_box_ism_inc={}
            geo_position_cellsPoints_masse_box_ism_exc={}
            for box_i,box_el in enumerate(Variable.x_frame_min):
                my_dict_name = create_box_file_dict_name(Variable.x_frame_min[box_i],
                                                         Variable.x_frame_max[box_i],
                                                         Variable.y_frame_min[box_i],
                                                         Variable.y_frame_max[box_i])
                geo_position_f_box_ism_inc[my_dict_name] = np.empty((numberOf3Delem,))
                geo_position_f_box_ism_exc[my_dict_name] = np.empty((numberOf3Delem,))
            #geo_position_masse_box_inc = np.empty((numberOf3Delem,))
            #geo_position_masse_box_exc = np.empty((numberOf3Delem,))
                geo_position_cellsPoints_masse_box_ism_inc[my_dict_name] = np.empty((numberOf3Delem,))
                geo_position_cellsPoints_masse_box_ism_exc[my_dict_name] = np.empty((numberOf3Delem,))
        if(Variable.frame_jet):
            geo_position_f_box_jet_inc = {}
            geo_position_f_box_jet_exc = {}
            ...
            for box_i,box_el in enumerate(Variable.x_frame_min):
                my_dict_name = create_box_file_dict_name(Variable.x_frame_min[box_i],
                                                         ...
                                                         Variable.y_frame_min[box_i],
                                                         Variable.y_frame_max[box_i])
                geo_position_f_box_jet_inc[my_dict_name] = np.empty((numberOf3Delem,))
                geo_position_f_box_jet_exc[my_dict_name] = np.empty((numberOf3Delem,))
            #geo_position_masse_box_inc = np.empty((numberOf3Delem,))
            #geo_position_masse_box_exc = np.empty((numberOf3Delem,))
                geo_position_cellsPoints_masse_box_jet_inc[my_dict_name] = np.empty((numberOf3Delem,))
                geo_position_cellsPoints_masse_box_jet_exc[my_dict_name] = np.empty((numberOf3Delem,))
    ...
    def set_attributes(self,output,my_variables):   
        ...
        if(my_variables.frame_the_box):
            self.geo_position_f_box_inc = {}
            self.geo_position_f_box_exc = {}
            self.geo_position_cellsPoints_masse_box_inc={}
            self.geo_position_cellsPoints_masse_box_exc={}
            for box_i,box_el in enumerate(my_variables.x_frame_min):
                ...
                self.geo_position_f_box_inc[my_dict_name] = np.empty((self.numberOf3Delem,))
                self.geo_position_f_box_exc[my_dict_name] = np.empty((self.numberOf3Delem,))
                #geo_position_masse_box_inc = np.empty((numberOf3Delem,))
                #geo_position_masse_box_exc = np.empty((numberOf3Delem,))
                self.geo_position_cellsPoints_masse_box_inc[my_dict_name] = np.empty((self.numberOf3Delem,))
                self.geo_position_cellsPoints_masse_box_exc[my_dict_name] = np.empty((self.numberOf3Delem,))                    
            if(my_variables.frame_ism):
                self.geo_position_f_box_ism_inc = {}
                self.geo_position_f_box_ism_exc = {}
                self.geo_position_cellsPoints_masse_box_ism_inc={}
                self.geo_position_cellsPoints_masse_box_ism_exc={}
                for box_i,box_el in enumerate(my_variables.x_frame_min):
                    my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                             my_variables.x_frame_max[box_i],
                                                             my_variables.y_frame_min[box_i],
                                                             my_variables.y_frame_max[box_i])
                    self.geo_position_f_box_ism_inc[my_dict_name] = np.empty((self.numberOf3Delem,))
                    self.geo_position_f_box_ism_exc[my_dict_name] = np.empty((self.numberOf3Delem,))
                #geo_position_masse_box_inc = np.empty((numberOf3Delem,))
                #geo_position_masse_box_exc = np.empty((numberOf3Delem,))
                    self.geo_position_cellsPoints_masse_box_ism_inc[my_dict_name] = np.empty((self.numberOf3Delem,))
                    self.geo_position_cellsPoints_masse_box_ism_exc[my_dict_name] = np.empty((self.numberOf3Delem,))
            if(my_variables.frame_jet):
                self.geo_position_f_box_jet_inc = {}
                self.geo_position_f_box_jet_exc = {}
                self.geo_position_cellsPoints_masse_box_jet_inc={}
                self.geo_position_cellsPoints_masse_box_jet_exc={}
                for box_i,box_el in enumerate(my_variables.x_frame_min):
                    my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                             my_variables.x_frame_max[box_i],
                                                             my_variables.y_frame_min[box_i],
                                                             my_variables.y_frame_max[box_i])
                    self.geo_position_f_box_jet_inc[my_dict_name] = np.empty((self.numberOf3Delem,))
                    self.geo_position_f_box_jet_exc[my_dict_name] = np.empty((self.numberOf3Delem,))
                #geo_position_masse_box_inc = np.empty((numberOf3Delem,))
                #geo_position_masse_box_exc = np.empty((numberOf3Delem,))
                    self.geo_position_cellsPoints_masse_box_jet_inc[my_dict_name] = np.empty((self.numberOf3Delem,))
                    self.geo_position_cellsPoints_masse_box_jet_exc[my_dict_name] = np.empty((self.numberOf3Delem,))                
...
class Railgun:
    ...
    cells_Masse_ISM_inc = None
    cells_Masse_ISM_exc = None
    cells_Masse_jet_inc = None
    cells_Masse_jet_exc = None
    cells_Masse_box_ism_inc = None
    cells_Masse_box_ism_exc = None
    cells_Masse_box_jet_inc = None
    cells_Masse_box_jet_exc = None
    ...
    f_box_2D_inc = None
    f_box_2D_exc = None
    f_box_2D_ism_inc = None
    f_box_2D_ism_exc = None
    f_box_2D_jet_inc = None
    f_box_2D_jet_exc = None    
...
class Sky_Grid:
    ...
    masse_arrays_box_inc=None
    masse_arrays_box_exc=None
    masse_arrays_box_ism_inc=None
    masse_arrays_box_ism_exc=None
    masse_arrays_box_jet_inc=None
    masse_arrays_box_jet_exc=None
    
    masse_arrays = np.zeros((nx, ny, nz), dtype=float)
    volume_arrays = np.zeros((nx, ny, nz), dtype=float)
    masse_arrays_ism = np.zeros((nx, ny, nz), dtype=float)
    masse_arrays_jet = np.zeros((nx, ny, nz), dtype=float)
    if(Variable.frame_the_box): 
        masse_arrays_box_inc={}
        masse_arrays_box_exc={}
        for box_i,box_el in enumerate(Variable.x_frame_min):
            my_dict_name = create_box_file_dict_name(Variable.x_frame_min[box_i],
                                                     Variable.x_frame_max[box_i],
                                                     Variable.y_frame_min[box_i],
                                                     Variable.y_frame_max[box_i])        
            masse_arrays_box_inc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)
            masse_arrays_box_exc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)
        if(Variable.frame_ism):
            masse_arrays_box_ism_inc={}
            masse_arrays_box_ism_exc={}
            for box_i,box_el in enumerate(Variable.x_frame_min):
                my_dict_name = create_box_file_dict_name(Variable.x_frame_min[box_i],
                                                         Variable.x_frame_max[box_i],
                                                         Variable.y_frame_min[box_i],
                                                         Variable.y_frame_max[box_i])
                masse_arrays_box_ism_inc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)
                masse_arrays_box_ism_exc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)
        if(Variable.frame_jet):
            masse_arrays_box_jet_inc={}
            masse_arrays_box_jet_exc={}
            for box_i,box_el in enumerate(Variable.x_frame_min):
                my_dict_name = create_box_file_dict_name(Variable.x_frame_min[box_i],
                                                         Variable.x_frame_max[box_i],
                                                         Variable.y_frame_min[box_i],
                                                         Variable.y_frame_max[box_i])
                masse_arrays_box_jet_inc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)
                masse_arrays_box_jet_exc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)                            
    if(Variable.floor_ism):
    ...
    def set_attributes(self,my_variables):
        ...
        self.volume_arrays = np.zeros((self.nx, self.ny, self.nz), dtype=float)
        ...
        if(my_variables.frame_the_box): 
            self.masse_arrays_box_inc={}
            self.masse_arrays_box_exc={}
            for box_i,box_el in enumerate(my_variables.x_frame_min):
                my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                         my_variables.x_frame_max[box_i],
                                                         my_variables.y_frame_min[box_i],
                                                         my_variables.y_frame_max[box_i])                    
                self.masse_arrays_box_inc[my_dict_name] = np.zeros((self.nx, self.ny, self.nz), dtype=float)
                self.masse_arrays_box_exc[my_dict_name] = np.zeros((self.nx, self.ny, self.nz), dtype=float)
            if(my_variables.frame_ism):
                self.masse_arrays_box_ism_inc={}
                self.masse_arrays_box_ism_exc={}
                for box_i,box_el in enumerate(my_variables.x_frame_min):
                    my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                             my_variables.x_frame_max[box_i],
                                                             my_variables.y_frame_min[box_i],
                                                             my_variables.y_frame_max[box_i])
                    self.masse_arrays_box_ism_inc[my_dict_name] = np.zeros((self.nx, self.ny, self.nz), dtype=float)
                    self.masse_arrays_box_ism_exc[my_dict_name] = np.zeros((self.nx, self.ny, self.nz), dtype=float)
            if(my_variables.frame_jet):
                self.masse_arrays_box_jet_inc={}
                self.masse_arrays_box_jet_exc={}
                for box_i,box_el in enumerate(my_variables.x_frame_min):
                    my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                             my_variables.x_frame_max[box_i],
                                                             my_variables.y_frame_min[box_i],
                                                             my_variables.y_frame_max[box_i])
                    self.masse_arrays_box_jet_inc[my_dict_name] = np.zeros((self.nx, self.ny, self.nz), dtype=float)
                    self.masse_arrays_box_jet_exc[my_dict_name] = np.zeros((self.nx, self.ny, self.nz), dtype=float)                                            
        if(my_variables.floor_ism):
        ...
        if(my_variables.frame_the_box): 
            ...
            for box_i,box_el in enumerate(my_variables.x_frame_min):
                my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                         ...
                                                         my_variables.y_frame_min[box_i],
                                                         my_variables.y_frame_max[box_i])                    
                self.masse_arrays_box_inc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)
                self.masse_arrays_box_exc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)
            if(my_variables.frame_ism):
                self.masse_arrays_box_ism_inc={}
                self.masse_arrays_box_ism_exc={}
                for box_i,box_el in enumerate(my_variables.x_frame_min):
                    my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                             ...
                                                             my_variables.y_frame_min[box_i],
                                                             my_variables.y_frame_max[box_i])
                    self.masse_arrays_box_ism_inc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)
                    self.masse_arrays_box_ism_exc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)
            if(my_variables.frame_jet):
                self.masse_arrays_box_jet_inc={}
                self.masse_arrays_box_jet_exc={}
                for box_i,box_el in enumerate(my_variables.x_frame_min):
                    my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                             ...
                                                             my_variables.y_frame_min[box_i],
                                                             my_variables.y_frame_max[box_i])
                    self.masse_arrays_box_jet_inc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)
                    self.masse_arrays_box_jet_exc[my_dict_name] = np.zeros((nx, ny, nz), dtype=float)                                                           
        if(my_variables.floor_ism):
            self.masse_arrays_ism_inc = np.zeros((nx, ny, nz), dtype=float)
            self.masse_arrays_ism_exc = np.zeros((nx, ny, nz), dtype=float)
        if(my_variables.floor_jet):
            self.masse_arrays_jet_inc = np.zeros((nx, ny, nz), dtype=float)
            self.masse_arrays_jet_exc = np.zeros((nx, ny, nz), dtype=float)
...
class MV_spectrum:
    ...
    masse_arrays = np.zeros((nz), dtype=float)
    masse_arrays_ism = np.zeros((nz), dtype=float)
    masse_arrays_jet = np.zeros((nz), dtype=float)
    if(Variable.frame_the_box): 
        masse_arrays_box_inc={}
        masse_arrays_box_exc={}
        for box_i,box_el in enumerate(Variable.x_frame_min):
            my_dict_name = create_box_file_dict_name(Variable.x_frame_min[box_i],
                                                     ...
                                                     Variable.y_frame_min[box_i],
                                                     Variable.y_frame_max[box_i])          
            masse_arrays_box_inc[my_dict_name] = np.zeros((nz), dtype=float)
            masse_arrays_box_exc[my_dict_name] = np.zeros((nz), dtype=float)
        if(Variable.frame_ism):
            masse_arrays_box_ism_inc={}
            masse_arrays_box_ism_exc={}
            for box_i,box_el in enumerate(Variable.x_frame_min):
                my_dict_name = create_box_file_dict_name(Variable.x_frame_min[box_i],
                                                         ...
                                                         Variable.y_frame_min[box_i],
                                                         Variable.y_frame_max[box_i])
                masse_arrays_box_ism_inc[my_dict_name] = np.zeros((nz), dtype=float)
                masse_arrays_box_ism_exc[my_dict_name] = np.zeros((nz), dtype=float)
        if(Variable.frame_jet):
            masse_arrays_box_jet_inc={}
            masse_arrays_box_jet_exc={}
            for box_i,box_el in enumerate(Variable.x_frame_min):
                my_dict_name = create_box_file_dict_name(Variable.x_frame_min[box_i],
                                                         ...
                                                         Variable.y_frame_min[box_i],
                                                         Variable.y_frame_max[box_i])
                masse_arrays_box_jet_inc[my_dict_name] = np.zeros((nz), dtype=float)
                masse_arrays_box_jet_exc[my_dict_name] = np.zeros((nz), dtype=float)
    if(Variable.floor_ism):
        masse_arrays_ism_inc = np.zeros((nz), dtype=float)
        masse_arrays_ism_exc = np.zeros((nz), dtype=float)
    if(Variable.floor_jet):
        masse_arrays_jet_inc = np.zeros((nz), dtype=float)
        masse_arrays_jet_exc = np.zeros((nz), dtype=float)
    ...
    def __init__(self):
        for varname in MV_spectrum.list_of_variables:
            setattr(self,varname,getattr(MV_spectrum,varname))
            
    def __repr__(self):
        o='{'
        for i,varname in enumerate(MV_spectrum.list_of_variables):
            o=o+varname+' : '+str(getattr(self,varname))
            if(i!=np.size(MV_spectrum.list_of_variables)-1): o+=' , '
        o+='}'
        return o
    ...
    def set_attributes(self,my_variables):
        ...
        self.masse_arrays = np.zeros((self.nz), dtype=float)
        self.masse_arrays_ism = np.zeros((self.nz), dtype=float)
        self.masse_arrays_jet = np.zeros((self.nz), dtype=float)
        if(my_variables.frame_the_box): 
            self.masse_arrays_box_inc = {}
            self.masse_arrays_box_exc = {}
            for box_i,box_el in enumerate(my_variables.x_frame_min):
                my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                         ...
                                                         my_variables.y_frame_min[box_i],
                                                         my_variables.y_frame_max[box_i])                      
                self.masse_arrays_box_inc[my_dict_name] = np.zeros((self.nz), dtype=float)
                self.masse_arrays_box_exc[my_dict_name] = np.zeros((self.nz), dtype=float)
            if(my_variables.frame_ism):
                self.masse_arrays_box_ism_inc={}
                self.masse_arrays_box_ism_exc={}
                for box_i,box_el in enumerate(my_variables.x_frame_min):
                    my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                             ...
                                                             my_variables.y_frame_min[box_i],
                                                             my_variables.y_frame_max[box_i])
                    self.masse_arrays_box_ism_inc[my_dict_name] = np.zeros((self.nz), dtype=float)
                    self.masse_arrays_box_ism_exc[my_dict_name] = np.zeros((self.nz), dtype=float)
            if(my_variables.frame_jet):
                self.masse_arrays_box_jet_inc={}
                self.masse_arrays_box_jet_exc={}
                for box_i,box_el in enumerate(my_variables.x_frame_min):
                    my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                             ...
                                                             my_variables.y_frame_min[box_i],
                                                             my_variables.y_frame_max[box_i])
                    self.masse_arrays_box_jet_inc[my_dict_name] = np.zeros((self.nz), dtype=float)
                    self.masse_arrays_box_jet_exc[my_dict_name] = np.zeros((self.nz), dtype=float)                
        if(my_variables.floor_ism):
            self.masse_arrays_ism_inc = np.zeros((self.nz), dtype=float)
            self.masse_arrays_ism_exc = np.zeros((self.nz), dtype=float)
        if(my_variables.floor_jet):
            self.masse_arrays_jet_inc = np.zeros((self.nz), dtype=float)
            self.masse_arrays_jet_exc = np.zeros((self.nz), dtype=float)
    ...
    def redefine_nz_arrays(self,my_variables,nz):
        self.masse_arrays = np.zeros((nz), dtype=float)
        self.masse_arrays_ism = np.zeros((nz), dtype=float)
        self.masse_arrays_jet = np.zeros((nz), dtype=float)
        if(my_variables.frame_the_box): 
            self.masse_arrays_box_inc = {}
            self.masse_arrays_box_exc = {}            
            for box_i,box_el in enumerate(my_variables.x_frame_min):
                my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                         ...
                                                         my_variables.y_frame_min[box_i],
                                                         my_variables.y_frame_max[box_i])                                  
                self.masse_arrays_box_inc[my_dict_name] = np.zeros((nz), dtype=float)
                self.masse_arrays_box_exc[my_dict_name] = np.zeros((nz), dtype=float)
            if(my_variables.frame_ism):
                self.masse_arrays_box_ism_inc={}
                self.masse_arrays_box_ism_exc={}
                for box_i,box_el in enumerate(my_variables.x_frame_min):
                    my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                             ...
                                                             my_variables.y_frame_min[box_i],
                                                             my_variables.y_frame_max[box_i])
                    self.masse_arrays_box_ism_inc[my_dict_name] = np.zeros((nz), dtype=float)
                    self.masse_arrays_box_ism_exc[my_dict_name] = np.zeros((nz), dtype=float)
            if(my_variables.frame_jet):
                self.masse_arrays_box_jet_inc={}
                self.masse_arrays_box_jet_exc={}
                for box_i,box_el in enumerate(my_variables.x_frame_min):
                    my_dict_name = create_box_file_dict_name(my_variables.x_frame_min[box_i],
                                                             ...
                                                             my_variables.y_frame_min[box_i],
                                                             my_variables.y_frame_max[box_i])
                    self.masse_arrays_box_jet_inc[my_dict_name] = np.zeros((nz), dtype=float)
                    self.masse_arrays_box_jet_exc[my_dict_name] = np.zeros((nz), dtype=float)                 
        if(my_variables.floor_ism):
            self.masse_arrays_ism_inc = np.zeros((nz), dtype=float)
            self.masse_arrays_ism_exc = np.zeros((nz), dtype=float)
        if(my_variables.floor_jet):
            self.masse_arrays_jet_inc = np.zeros((nz), dtype=float)
            self.masse_arrays_jet_exc = np.zeros((nz), dtype=float)
