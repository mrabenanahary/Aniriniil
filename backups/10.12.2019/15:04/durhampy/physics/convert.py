...
acceptable_inputs =     [   'EnergieTONu',
                            'EnergieTOWavelength',
                            'IntegratedIntensityTOSurfaceBrightness',
                            'IntegratedIntensityTOSurfaceFlux',
                            'NuTOEnergie',
                            'NuTOWavelength',
                            'SurfaceBrightnessTOIntegratedIntensity',
                            'SurfaceBrightnessTOSurfaceFlux',
                            'SurfaceFluxTOIntegratedIntensity',
                            'SurfaceFluxTOSurfaceBrightness',
                            'WavelengthTOEnergie',
                            'WavelengthTONu']

name_to_function_map = {    "frequency-to-wavelength" : "NuTOWavelength",
                            "frequency-to-energy" : "NuTOEnergie",
                            "wavelength-to-frequency" : "WavelengthTONu",
                            "wavelength-to-energy" : "WavelengthTOEnergie",
                            "energy-to-frequency" :  "EnergieTONu",
                            "energy-to-wavelength" : "EnergieTOWavelength",
                            "surfaceBrightness-to-integratedIntensity" : "SurfaceBrightnessTOIntegratedIntensity",
                            "surfaceFlux-to-integratedIntensity": "SurfaceFluxTOIntegratedIntensity",
                            "integratedIntensity-to-surfaceFlux" : "IntegratedIntensityTOSurfaceFlux",
                            "surfaceBrightness-to-surfaceFlux" : "SurfaceBrightnessTOSurfaceFlux",
                            "surfaceFlux-to-surfaceBrightness" : "SurfaceFluxTOSurfaceBrightness",
                            "integratedIntensity-to-surfaceBrightness" : "IntegratedIntensityTOSurfaceBrightness"
                        }
...
class LocalConverters:    
    #Attention, ne pas rajouter de méthodes commençant par '__' lorsque vous rajoutez un convertisseur
    ...
    #De même, ne mettre donc que des convertisseurs sous cette classe LocalConverters et rien d'autre
    #pour ne pas corrompre le listing automatique de la liste des convertisseurs présents sous 
...
"==================NE PAS BOUGER CECI!=============="
convLocal_function_names = [ m for m in dir(LocalConverters) if not m.startswith('__')]
...
def convertisseurPhysique(
    ...
    if(  i_input == acceptable_inputs_to_num_map["NuTOWavelength"]):
        out_put = LocalConverters.NuTOWavelength(                        nu=frequence, 
                                                         unit_of_nu=unite_de_frequence,
                                                         unit_of_wavelength=unite_de_longueur_d_onde,
                                                         return_quantities=retourner_qtes_physiques)
        
    elif(i_input == acceptable_inputs_to_num_map["NuTOEnergie"]):
        out_put = LocalConverters.NuTOEnergie(                           nu=frequence,
                                                         unit_of_nu=unite_de_frequence,
                                                         unit_of_energy=unite_d_energie,
                                                         return_quantities=retourner_qtes_physiques)
        
    elif(i_input == acceptable_inputs_to_num_map["WavelengthTONu"]):
        out_put = LocalConverters.WavelengthTONu(                        wavelength=longueur_d_onde,
                                                         unit_of_wavelength=unite_de_longueur_d_onde,
                                                         unit_of_nu=unite_de_frequence,
                                                         return_quantities=retourner_qtes_physiques)
        
    elif(i_input == acceptable_inputs_to_num_map["WavelengthTOEnergie"]):
        out_put = LocalConverters.WavelengthTOEnergie(                   wavelength=longueur_d_onde,
                                                         unit_of_wavelength=unite_de_longueur_d_onde, 
                                                         unit_of_energy=unite_d_energie, 
                                                         return_quantities=retourner_qtes_physiques)
        
    elif(i_input == acceptable_inputs_to_num_map["EnergieTONu"]):
        out_put = LocalConverters.EnergieTONu(                           energy=energie,
                                                         unit_of_energy=unite_d_energie,
                                                         unit_of_nu=unite_de_frequence,
                                                         return_quantities=retourner_qtes_physiques)
        
    elif(i_input == acceptable_inputs_to_num_map["EnergieTOWavelength"]):
        out_put = LocalConverters.EnergieTOWavelength(                   energy=energie,
                                                         unit_of_energy=unite_d_energie,
                                                         unit_of_wavelength=unite_de_longueur_d_onde, 
                                                         return_quantities=retourner_qtes_physiques)
        
    elif(i_input == acceptable_inputs_to_num_map["SurfaceBrightnessTOIntegratedIntensity"]):
        out_put = LocalConverters.SurfaceBrightnessTOIntegratedIntensity(surface_brightness=brillance_de_surface,
    ...
    elif(i_input == acceptable_inputs_to_num_map["SurfaceFluxTOIntegratedIntensity"]):
        out_put = LocalConverters.SurfaceFluxTOIntegratedIntensity(      surface_flux=flux_de_surface,
                                                         wavelength=longueur_d_onde, 
    ...
    elif(i_input == acceptable_inputs_to_num_map["IntegratedIntensityTOSurfaceFlux"]):
        out_put = LocalConverters.IntegratedIntensityTOSurfaceFlux(      integrated_intensity=intensite_integree,
                                                         theta_s=theta_source,
                                                         wavelength=longueur_d_onde,
                                                         unit_of_integrated_intensity = unite_d_intensite_integree,
                                                         unit_of_theta_s = unite_de_theta_source,
    ...
    elif(i_input == acceptable_inputs_to_num_map["SurfaceBrightnessTOSurfaceFlux"]):
        out_put = LocalConverters.SurfaceBrightnessTOSurfaceFlux(        surface_brightness=brillance_de_surface,
                                                         theta_s=theta_source,
                                                         unit_of_surface_brightness = unite_de_brillance_de_surface,
                                                         unit_of_theta_s = unite_de_theta_source,
                                                         unit_of_surface_flux = unite_de_flux_surfacique, 
    ...
    elif(i_input == acceptable_inputs_to_num_map["SurfaceFluxTOSurfaceBrightness"]):
        out_put = LocalConverters.SurfaceFluxTOSurfaceBrightness(        surface_flux=flux_de_surface,
                                                         theta_s=theta_source,
                                                         unit_of_surface_flux=unite_de_flux_surfacique,
                                                         unit_of_theta_s=unite_de_theta_source,
                                                         unit_of_surface_brightness=unite_de_brillance_de_surface,
    ...
    elif(i_input == acceptable_inputs_to_num_map["IntegratedIntensityTOSurfaceBrightness"]):
        out_put = LocalConverters.IntegratedIntensityTOSurfaceBrightness(integrated_intensity=intensite_integree,
                                                         wavelength=longueur_d_onde,
                                                         unit_of_integrated_intensity=unite_d_intensite_integree,
                                                         unit_of_wavelength=unite_de_longueur_d_onde,
                                                         unit_of_surface_brightness=unite_de_brillance_de_surface,
...
if __name__ == "__main__":
    ...
    a = LocalConverters.WavelengthTONu(wavelength=145,unit_of_wavelength=u.micrometer,unit_of_nu=u.GHz)
    print("a=",a)
    
    
    b = LocalConverters.NuTOWavelength(nu=2067.5342,unit_of_wavelength=u.micrometer,unit_of_nu=u.GHz,return_quantities=True)
    print("b=",b)
    ...
    c = LocalConverters.SurfaceBrightnessTOIntegratedIntensity(1,2,unit_of_surface_brightness=u.erg/u.sr/u.cm**2/u.s,unit_of_wavelength=u.micrometer,return_quantities=retQtt, unit_of_integrated_intensity = u.K * u.km/u.s)
    print("c=",c)
    
    d = LocalConverters.SurfaceBrightnessTOIntegratedIntensity(1,2,unit_of_surface_brightness=u.watt/u.sr/u.m**2,unit_of_wavelength=u.m, return_quantities=retQtt, unit_of_integrated_intensity = u.K * u.km/u.s)
    print("d=",d)
    print((3.62146e-2)*((1e7)/(1e4))*((2e6)**3))
    
    
    e =  LocalConverters.SurfaceFluxTOIntegratedIntensity(1e-6,145,10,unit_of_surface_flux = dixMoins18Watt/u.m**2, unit_of_wavelength = u.micrometer, unit_of_theta_s = u.arcsecond, unit_of_integrated_intensity = u.K*u.km/u.s)
    print("e=",e)
    
    f = LocalConverters.SurfaceFluxTOIntegratedIntensity(1e-6,145,10,unit_of_surface_flux = u.erg/u.s/u.cm**2,unit_of_wavelength=u.millimeter,unit_of_theta_s=u.radian,return_quantities=True, unit_of_integrated_intensity = u.K*u.km/u.s)
    print("f=",f)
    print(1.54076*1e-6*1e-6*((1e-7*1e18)/1e-4)*1e-9*((145*1e6)**3) / ((10*206265)**2))
    
    g = LocalConverters.IntegratedIntensityTOSurfaceFlux(1,1,1,                                          unit_of_integrated_intensity = u.K*u.km/u.s,
                                          unit_of_theta_s = u.arcsecond,
                                          unit_of_wavelength = u.micrometer,
                                          unit_of_surface_flux = dixMoins18Watt/u.m**2)
    print('g=',g)
    
    h = LocalConverters.IntegratedIntensityTOSurfaceFlux(1e7,2,12,unit_of_integrated_intensity = u.K*u.m/u.s,unit_of_theta_s = u.radian,unit_of_wavelength = u.mm,return_quantities=True,unit_of_surface_flux = dixMoins18Watt/u.m**2)
    print("h=",h)
    print(6.49028e5*1e7*1e-3*(((2*206265)**2)/((12*1e-3*1e6)**3)), "<=> h_unit / h_a_la_main = ", h/(6.49028e5*1e7*1e-3*(((2*206265)**2)/((12*1e-3*1e6)**3))))
    
    i = LocalConverters.SurfaceBrightnessTOSurfaceFlux(1,1,unit_of_surface_brightness = u.erg/u.sr/u.cm**2/u.s,
                                          unit_of_theta_s = u.arcsecond,
                                          unit_of_surface_flux = dixMoins18Watt/u.m**2)
    print('i=',i)
    
    j = LocalConverters.SurfaceBrightnessTOSurfaceFlux(1e7,12,unit_of_surface_brightness = u.watt/u.sr/u.m**2,unit_of_theta_s = u.radian,unit_of_surface_flux=dixMoins18Watt/u.m**2, return_quantities=True)
    print("j=",j)
    print((2.35044e4)*1e7*(1e7/1e4)*(12*206265)**2)
    
    k = LocalConverters.SurfaceFluxTOSurfaceBrightness(1,1,unit_of_surface_flux = dixMoins18Watt/u.m**2,
                                          unit_of_theta_s = u.arcsecond,
                                          unit_of_surface_brightness = u.erg/u.sr/u.cm**2/u.s)
    print('k=',k)
    
    l = LocalConverters.SurfaceFluxTOSurfaceBrightness(1e7,12,unit_of_surface_flux = u.erg/u.s/u.cm**2,unit_of_theta_s = u.radian,return_quantities=True,unit_of_surface_brightness = u.erg/u.sr/u.cm**2/u.s)
    print("l=",l)
    print(4.25452e-5*1e7*((1e-7*1e18)/(1e-4))*1/((12*206265)**2))
    
    m = LocalConverters.IntegratedIntensityTOSurfaceBrightness(1,1,unit_of_integrated_intensity = u.K*u.km/u.s,
                                          unit_of_wavelength = u.micrometer,
                                          unit_of_surface_brightness = u.erg/u.sr/u.cm**2/u.s)
    print('m=',m)
    
    n = LocalConverters.IntegratedIntensityTOSurfaceBrightness(1e7,12,unit_of_integrated_intensity = u.K*u.m/u.s,unit_of_wavelength = u.millimeter,return_quantities=True,unit_of_surface_brightness = u.erg/u.sr/u.cm**2/u.s)
    print("n=",n)
    print(27.6131*1e7*(1e-3)*1/((12*1e-3*1e6)**3))
    
    aa = LocalConverters.NuTOEnergie(10418.241360755546,return_quantities=False)
    bb = LocalConverters.NuTOEnergie(10418.241360755546,return_quantities=True)
    print(aa)
    print(bb)
    
    cc = LocalConverters.EnergieTONu(500,return_quantities=False)
    dd = LocalConverters.EnergieTONu(500,return_quantities=True)
    print(cc)
    print(dd)
    
    
    ee = LocalConverters.NuTOEnergie(10418.241360755546e9,unit_of_nu = u.Hz,return_quantities=False)
    ff = LocalConverters.NuTOEnergie(10418.241360755546e9,unit_of_nu = u.Hz,return_quantities=True)
    print(ee)
    print(ff)
    
    gg = LocalConverters.EnergieTONu(0.043086404716694296,return_quantities=False,unit_of_energy=u.eV)
    hh = LocalConverters.EnergieTONu(0.043086404716694296,return_quantities=True,unit_of_energy=u.eV)
    print(gg)
    print(hh)
    
    del aa,bb,cc,dd,ee,ff,gg,hh
    
    aa = LocalConverters.WavelengthTOEnergie(28.77583,return_quantities=False)
    bb = LocalConverters.WavelengthTOEnergie(28.77583,return_quantities=True)
    print(aa)
    print(bb)
    
    cc = LocalConverters.EnergieTOWavelength(500,return_quantities=False)
    dd = LocalConverters.EnergieTOWavelength(500,return_quantities=True)
    print(cc)
    print(dd)
    
    
    ee = LocalConverters.WavelengthTOEnergie(28.77583e-6,unit_of_wavelength = u.m,return_quantities=False)
    ff = LocalConverters.WavelengthTOEnergie(28.77583e-6,unit_of_wavelength = u.m,return_quantities=True)
    print(ee)
    print(ff)
    
    gg = LocalConverters.EnergieTOWavelength(0.043086404716694296,return_quantities=False,unit_of_energy=u.eV)
    hh = LocalConverters.EnergieTOWavelength(0.043086404716694296,return_quantities=True,unit_of_energy=u.eV)
    print(gg)
    print(hh)
    ...
    print(convertisseurPhysique(type_de_conversion=Type, frequence=10418.241360755546e9,unite_de_frequence=u.Hz,unite_de_longueur_d_onde=u.m, retourner_qtes_physiques=False),
          ...
          LocalConverters.NuTOWavelength(nu=10418.241360755546e9,unit_of_wavelength=u.m,unit_of_nu=u.Hz,return_quantities=True))
    ...
    print(convertisseurPhysique(type_de_conversion=Type, frequence=10418.241360755546e9,unite_de_frequence=u.Hz,unite_d_energie=u.erg, retourner_qtes_physiques=False),
          ...
          LocalConverters.NuTOEnergie(nu=10418.241360755546e9,unit_of_energy=u.erg,unit_of_nu=u.Hz,return_quantities=True))
    ...
    print(convertisseurPhysique(type_de_conversion=Type, longueur_d_onde=145e-6, unite_de_longueur_d_onde=u.m, unite_de_frequence=u.Hz, retourner_qtes_physiques=False),
          ...
          LocalConverters.WavelengthTONu(wavelength=145e-6,unit_of_wavelength=u.m,unit_of_nu=u.Hz, return_quantities=True))
    
    """wavelength-to-energy"""
    Type = "WavelengthTOEnergie"
    print(convertisseurPhysique(type_de_conversion=Type, longueur_d_onde=145e-6, unite_de_longueur_d_onde=u.m, unite_d_energie=u.erg, retourner_qtes_physiques=False),
          ...
          LocalConverters.WavelengthTOEnergie(wavelength=145e-6,unit_of_wavelength=u.m,unit_of_energy=u.erg, return_quantities=True))
    ...
    print(convertisseurPhysique(type_de_conversion=Type, energie=0.043086404716694296, unite_d_energie=u.eV, unite_de_frequence=u.Hz, retourner_qtes_physiques=False),
          ...
          LocalConverters.EnergieTONu(0.043086404716694296,return_quantities=True,unit_of_energy=u.eV,unit_of_nu=u.Hz))
    
    """energy-to-wavelength"""
    Type = "EnergieTOWavelength"
    print(convertisseurPhysique(type_de_conversion=Type, energie=0.043086404716694296, unite_d_energie=u.eV, unite_de_longueur_d_onde=u.m, retourner_qtes_physiques=False),
          ...
          LocalConverters.EnergieTOWavelength(0.043086404716694296,return_quantities=True,unit_of_energy=u.eV,unit_of_wavelength=u.m))
    ...
    Type = "SurfaceBrightnessTOIntegratedIntensity"
    print('\n',
    ...
    Type = "SurfaceFluxTOIntegratedIntensity"
    print('\n',
    ...
    Type = "IntegratedIntensityTOSurfaceFlux"
    print('\n',
    ...
    Type = "SurfaceBrightnessTOSurfaceFlux"
    print('\n',
    ...
    Type = "SurfaceFluxTOSurfaceBrightness"
    print('\n',
    ...
    Type = "IntegratedIntensityTOSurfaceBrightness"
    print('\n',
