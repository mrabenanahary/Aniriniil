...
def variable_set_units(my_variables,item):
        if(item=='geo_angle_i'): my_variables.geo_angle_i = (my_variables.geo_angle_i * my_variables.unit_angle_i).to_value(u.rad)
        
        if(item=='voxels_dz'): my_variables.voxels_dz = (my_variables.voxels_dz*my_variables.unit_voxels_dz).to_value(u.cm/u.s)
        if(item=='voxels_z_max'): my_variables.voxels_z_max = (my_variables.voxels_z_max*my_variables.unit_voxels_z_max).to_value(u.cm/u.s)
        if(item=='voxels_z_min'): my_variables.voxels_z_min = (my_variables.voxels_z_min*my_variables.unit_voxels_z_min).to_value(u.cm/u.s)
        
        if(item=='each_percent_frac'): my_variables.each_percent_frac = my_variables.each_percent/100
        if(item=='number_of_voxels'): my_variables.number_of_voxels = my_variables.voxels_nGamma_cells*my_variables.voxels_nGamma_cells
    ...
    with open(filename) as file:
        ...
        for item,doc in params_dict.items():
            if(item in Variable.list_of_variables):
                ...
                a_variable = variable_set_units(a_variable,item)
            else:
