            ...
            if os.path.exists(nameFiles[i]): 
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    if(iAx2%16==0):
                        ...
                        figExcit.append([])
                        axTable.append([[],[],[],[]])
                        figExcit[-1], [axTable[-1][0],axTable[-1][1],axTable[-1][2],axTable[-1][3]] = plt.subplots(1,4, sharey=ShareY,figsize=(28,7))
                        ...
                        for element in [axTable[-1][0],axTable[-1][1],axTable[-1][2],axTable[-1][3]]:
                            element.grid(True)
                    ...
                    iAx2=iAx2+1
                    if(iAx2%8==0):
                        lgd = axTable[-1][iAx].legend(fontsize=14,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.45))
                        iAx=iAx+1
                    if(iAx2%16==0):
                        os.chdir(originalDir)          
            ...
            if os.path.exists(nameFiles[i]): 
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    ...
                    if(iAx2%8==0): 
                        ...
                        #lgd = axTable[iFig][iAx3].legend(fontsize=14,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.45))
                        iAx=iAx+1
                        iAx3=iAx3+1
                    if(iAx2%16==0):
                        os.chdir(originalDir)          
...
if(iAx2%16!=0):
    ...
    lgd = axTable[iFig][iAx3+1].legend(fontsize=14,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.45))
    plt.show()
    plt.close()  
        ...
        for l,vt in enumerate(vitesseTime[k]):
            ...
            if os.path.exists(nameFiles[i]): 
                ...
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    ...
                    #axTable[-1][iAx].plot(X,Y,label=dirdir+vt,alpha=0.65,linewidth=linewdthsize[iAx2%8])
                    iAx2=iAx2+1
                    if(iAx2%8==0):
                        #lgd = axTable[-1][iAx].legend(fontsize=14,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.45))
                        iAx=iAx+1
...
if(iAx2%16!=0):
    lgd = axTable[iFig][iAx].legend(fontsize=14,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.45))
    figExcit[iFig].savefig("CO_TdV_{}_noPdF.pdf".format(iAx2), bbox_inches='tight')
    plt.show()
    plt.close()                        