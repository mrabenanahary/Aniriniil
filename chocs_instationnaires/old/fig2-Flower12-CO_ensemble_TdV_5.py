...
for i,directory in enumerate(dirFiles):       
    figData[directory]={}
    os.chdir(directory)
    dirlist.append(os.popen('ls -1d cj-n5-b1').read().split())
        ...
        for l,vt in enumerate(vitesseTime[k]):
            ...
            if os.path.exists(nameFiles[i]): 
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    ...
                    iAx2=iAx2+1
                    if(iAx2%4==0):
                        lgd = axTable[-1][iAx].legend(fontsize=9,loc='best')
                        iAx=iAx+1
                    if(iAx2%16==0):
                        os.chdir(originalDir)          
            ...
            if os.path.exists(nameFiles[i]): 
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    ...
                    if(iAx2%4==0): 
                        ...
                        #lgd = axTable[iFig][iAx3].legend(fontsize=9,loc='best')
                        iAx=iAx+1
                        iAx3=iAx3+1
                    if(iAx2%16==0):
                        os.chdir(originalDir)          
        ...
        for l,vt in enumerate(vitesseTime[k]):
            ...
            if os.path.exists(nameFiles[i]): 
                ...
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    ...
                    if(iAx2%16==0):
                        os.chdir(originalDir)
                        figExcit[iFig].savefig("CO_TdV_{}b.pdf".format(iAx2), bbox_inches='tight')
                        plt.show()
                        plt.close()                        
                        os.chdir(currentDirLvL3)
                        iFig=iFig+1
...
if(iAx2%16!=0):
    lgd = axTable[-1][iAx].legend(fontsize=9,loc='best')
    figExcit[-1].savefig("CO_TdV_{}b.pdf".format(iAx2), bbox_inches='tight')
    plt.show()
    plt.close()                        
