        ...
        for l,vt in enumerate(vitesseTime[k]):
            ...
            if os.path.exists(nameFiles[i]): 
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    ...
                    iAx2=iAx2+1
                    if(iAx2%6==0):
                        lgd = axTable[-1][iAx].legend(fontsize=10,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.5))
                        iAx=iAx+1
                    if(iAx2%12==0):
                        os.chdir(originalDir)          
            ...
            if os.path.exists(nameFiles[i]): 
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    ...
                    if(iAx2%6==0): 
                        ...
                        #lgd = axTable[iFig][iAx3].legend(fontsize=10,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.5))
                        iAx=iAx+1
                        iAx3=iAx3+1
                    if(iAx2%12==0):
                        os.chdir(originalDir)          
...
if(iAx2%12!=0):
    ...
    lgd = axTable[iFig][iAx3+1].legend(fontsize=10,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.5))
    plt.show()
    plt.close()  
        ...
        for l,vt in enumerate(vitesseTime[k]):
            ...
            if os.path.exists(nameFiles[i]): 
                ...
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    ...
                    #axTable[-1][iAx].plot(X,Y,label=dirdir+vt,alpha=0.65,linewidth=linewdthsize[iAx2%6])
                    iAx2=iAx2+1
                    if(iAx2%6==0):
                        #lgd = axTable[-1][iAx].legend(fontsize=10,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.5))
                        iAx=iAx+1
...
if(iAx2%12!=0):
    lgd = axTable[iFig][iAx].legend(fontsize=10,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.5))
    figExcit[iFig].savefig("CO_TdV_{}_noPdF.pdf".format(iAx2), bbox_inches='tight')
    plt.show()
    plt.close()                        