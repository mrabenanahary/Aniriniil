            ...
            if os.path.exists(nameFiles[i]): 
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    if(iAx2%16==0):
                        ...
                        figExcit.append([])
                        axTable.append([[],[],[],[]])
                        figExcit[-1], [axTable[-1][0],axTable[-1][1],axTable[-1][2],axTable[-1][3]] = plt.subplots(1,4, sharey=True,figsize=(56,14))
                    ...
                    axTable[-1][iAx].plot(X,Y,'-o',markersize=2,label=dirdir+vt,alpha=0.65,linewidth=1.5)
                    iAx2=iAx2+1
                    if(iAx2%4==0):
                        lgd = axTable[-1][iAx].legend(fontsize=18,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.45))
                        iAx=iAx+1
                    if(iAx2%16==0):
                        os.chdir(originalDir)          
            ...
            if os.path.exists(nameFiles[i]): 
                for m,Yheader in enumerate(nameFilesYHeader[i]):
                    ...
                    if(iAx2%4==0): 
                        ...
                        #lgd = axTable[iFig][iAx3].legend(fontsize=18,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.45))
                        iAx=iAx+1
                        iAx3=iAx3+1
                    if(iAx2%16==0):
                        os.chdir(originalDir)          
                    ...
                    for l,vt in enumerate(vitesseTime[k]):
                        ...
                        if os.path.exists(nameFiles[i]): 
                            for m,Yheader in enumerate(nameFilesYHeader[i]):
                                ...
                                iAx2=iAx2+1
                                if(iAx2%4==0): 
                                    axTable[iFig][iAx3].scatter(X,Y,s=6,zorder=3,marker='D',label=LabelObs[n],facecolors='white',edgecolors='black')
