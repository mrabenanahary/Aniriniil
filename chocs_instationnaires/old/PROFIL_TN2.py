...
labeldirFiles = {}
vitDirFiles = [[19]]
ageDirFiles = [[[50,100,150,200,250,500,9.99e+99]]]
...
figRatio, [ax,ax2] = plt.subplots(1,2, sharey=False,figsize=(14,7))
...
ax.set_ylabel(r'$T_n$ (K) ')
ax.set_xlabel(r'$t_i$ (ans)')
ax2.grid(True,axis='both',alpha=0.5,zorder=0)
ax2.set_xlim(7,1000)
ax2.set_xscale(axTableXScale)
ax2.set_yscale(axTableYScale)
ax2.set_ylabel(r'$T_i$ (K) ',rotation=270)
ax2.set_xlabel(r'$t_i$ (ans)')
    ...
    for j,vitesse in enumerate(vitDirFiles[i]):        
        ...
        for k,tempsJ in enumerate((ageDirFiles[i])[j]):
            ...
            for line in asciiTemp:
                ((figDataNum[directory])[vitesse])[tempsJ]['Tn'].append(line[14])
...
for i,directory in enumerate(dirFiles):
    ...
    for j,vitesse in enumerate(vitDirFiles[i]):
        ...
        for k,tempsJ in enumerate((ageDirFiles[i])[j]):
            ...
            ax2.plot(X,Y2,'-',label=((labeldirFiles[directory])[vitesse])[tempsJ],zorder=len((ageDirFiles[i])[j])-k,color=labelTextColor[k])
            print(max(Y))
            Xline=np.full((100,),tempsJ)
            itempsJ=np.where(np.array(X)<tempsJ)[0][-1]
            Yline=np.linspace(0,Y[itempsJ],num=100)
            Xline2=np.full((100,),tempsJ)
            itempsJ2=np.where(np.array(X)<tempsJ)[0][-1]
            Yline2=np.linspace(0,Y2[itempsJ],num=100)
            ax.plot(Xline,Yline,'--',color='black',linewidth=1.0,zorder=1,alpha=0.5)
            ax2.plot(Xline2,Yline2,'--',color='black',linewidth=1.0,zorder=1,alpha=0.5)
            if(tempsJ<10000.): 
                ax.text(1.05*tempsJ,1.4*Y[itempsJ],r'\textbf{%s ans}' % (str(tempsJ)),color=labelTextColor[k],fontsize=12)
                ax2.text(1.05*tempsJ,1.4*Y2[itempsJ],r'\textbf{%s ans}' % (str(tempsJ)),color=labelTextColor[k],fontsize=12)
...
lgd = ax.legend(fontsize=14,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.45))
lgd = ax2.legend(fontsize=14,loc='lower center',ncol=2,bbox_to_anchor=(0.5, -0.45))

"""Enregistrement des figures"""
fileTitle="Tn_"+"{}".format(raieNomNum)
...
for i,directory in enumerate(dirFiles):
    for j,vitesse in enumerate(vitDirFiles[i]):
        for k,tempsJ in enumerate((ageDirFiles[i])[j]):
            if(tempsJ<10000.): 
                ax.text(tempsJ,1.5*ax.get_ylim()[0],r'%s' % (str(tempsJ)),color='black',fontsize=12,rotation=90)
                ax2.text(tempsJ,1.5*ax2.get_ylim()[0],r'%s' % (str(tempsJ)),color='black',fontsize=12,rotation=90)
            #ax.axvline(x=tempsJ,ymin=0,ymax=1,ls='--',color='grey',linewidth=0.5)
        l=l+1
