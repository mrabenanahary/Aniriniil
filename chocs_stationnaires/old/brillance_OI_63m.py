...
ax.set_ylabel(r'$\sum T_{mb}\Delta v$  (erg/s/$cm^2$/sr) '+r'{:1}'.format(raieNomNum))
...
for i,directory in enumerate(dirFiles):
    ...
    for vitesse in vitDirFiles[i]:
        ...
        temp = (figDataNum[directory])[vitesse]/(1.e-12*lambdaNumMicrons**3/(2.*KB)/1.e5)
        (figDataNum[directory])[vitesse]= temp
        print(directory,vitDirName,(figDataNum[directory])[vitesse])
        """Dénominateur"""
...
Y=[]
dirdir=[]
for i,directory in enumerate(figDataNum.keys()):
    X.append([])
    Y.append([])
    dirdir.append(directory)
    for vitesse in sorted((figDataNum[directory]).keys()):
        X[i].append(vitesse)
        Y[i].append((figDataNum[directory])[vitesse])
