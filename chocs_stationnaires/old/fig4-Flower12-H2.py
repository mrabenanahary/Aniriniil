...
labels=[r'$n_H=10^4 cm^{-3}$ $b=1$ $V_s=5 km.s^{-1}$ (choc C)',
           r'$n_H=10^4 cm^{-3}$ $b=1$ $V_s=10 km.s^{-1}$ (choc C)',
           r'$n_H=10^4 cm^{-3}$ $b=1$ $V_s=20 km.s^{-1}$ (choc J)',
           r'$n_H=10^4 cm^{-3}$ $b=1$ $V_s=25 km.s^{-1}$ (choc J)',
           r'$n_H=10^4 cm^{-3}$ $b=1$ $V_s=30 km.s^{-1}$ (choc J)'
           ]
...
for ii,densiteChamp in enumerate(dirlist):
    ...
    for kk,vitesse in enumerate(ulist):
        ...
        print("Y=",Y)
        #print(Y)
        ax.plot(E_up,Y,'-o',markersize=12,label=labels[kk])        
"""
...
plt.text(500, 33,r'$n_H=10^4$ $cm^{-3}$, $B=100$ $\mu G$',fontsize=20,weight='bold')    
"""
#observationnal data:
...
Xobs=np.array([E_up[0],
               ...
               E_up[5],
               E_up[6]])
Yobs=np.append(DiagramOrdonnee_log[0:5],DiagramOrdonnee_log[6:])
ax.scatter(Xobs,Yobs,zorder=6,s=125,marker='D',edgecolors='black',facecolors='yellow',label='Observations')
...
lgd = plt.legend(fontsize=18,loc='best')
"""
...
plt.savefig("fig4_H2_Excit_Diag_comparison_flower12.png", bbox_inches='tight')
plt.savefig("fig4_H2_Excit_Diag_comparison__flower12.pdf", bbox_inches='tight')
plt.show()    
plt.close()