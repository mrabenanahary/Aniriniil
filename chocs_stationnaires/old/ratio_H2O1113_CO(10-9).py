...
dirlistNum = []
vitesseTimeNum = []
lambdaNumMicrons=269.30 #a remplir si les données Yang 2017 sont activées
...
dirlistDen = []
vitesseTimeDen = []
lambdaDenMicrons=260.25 #a remplir si les données Yang 2017 sont activées
...
raieNomRatio = TelescopeNameNum+' '+raieNomNum+'\n'+'/'+TelescopeNameDen+' '+raieNomDen
nameFilesRatio = "ratio_CO_H2O.dat"
iRatio = 2 #numero de ligne Ascii
...
if(ActiverMyObs==True):
    ...
    ratioY = np.full(ratioX.shape,(fig4data[iRatio])[jRatio])#(fig4data[iRatio])[jRatio]
    #print(fig4data)
...
lgd = plt.legend(fontsize=7,loc='lower right',ncol=1,bbox_to_anchor=(1.05,-0.05))
