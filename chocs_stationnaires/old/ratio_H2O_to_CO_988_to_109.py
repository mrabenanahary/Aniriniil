...
ax.set_ylabel(r'rapport de $\sum T\mathrm{d}v$ 998GHz p-$H_2O$/CO(10-9)',fontsize=23,fontweight='extra bold')
...
ratio2Y = np.full(ratio2X.shape,(fig4data[3])[1])
...
for kk,vitesse in enumerate(ulist):
    ...
    chemin= vitesse + "/" + "pH2O_int_int_b.out"
    cheminb= vitesse + "/" + "CO_int_int.out"
    ...
    #print(Y)
    Y.append((fig4data[1])[1]/(fig4datab[9])[1])
...
ax.plot(ratio2X,ratio2Y,label=r'HIFI 998 GHz/APEX CO(10-9)')        
#ax.plot(ratio1X,ratio1Y,label=r'Observ. 1113 GHz/CO(3-2)')        

#1113 GHz <=> 53.4 K
#988 GHz <=> 100.8 K

chemina='./Table2_Yang17_pH2O.txt'
cheminb='./Table2_yang17_CO.txt'
indexLine1=4
indexLine2=-7
...
plt.savefig("ratio_H2O_to_CO_988_to_109.png", bbox_inches='tight')
plt.savefig("ratio_H2O_to_CO_988_to_109.pdf", bbox_inches='tight')
plt.show()    
plt.close()
