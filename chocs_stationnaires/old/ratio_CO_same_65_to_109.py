...
ax.set_ylabel(r'rapport de $\sum T\mathrm{d}v$ CO(10-9)/CO(6-5)',fontsize=23,fontweight='extra bold')
...
for kk,vitesse in enumerate(ulist):
    ...
    #print(Y)
    Y.append((fig4data[5])[1]/(fig4data[9])[1])
...
ax.plot(ratio2X,1./np.array(ratio2Y),label=r'CO(10-9)/CO(6-5) APEX')        
...
plt.savefig("ratio_CO_same_109_to_65.png", bbox_inches='tight')
plt.savefig("ratio_CO_same_109_to_65.pdf", bbox_inches='tight')
plt.show()    
plt.close()
