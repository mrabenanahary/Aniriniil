...
def sample_sky_plane_grid(my_variables,my_bazooka,my_sky_grid):
    ...
    if(my_variables.voxelling_scheme=='brute'):
        ...
        for h in range(my_sky_grid.nz):
            d = np.abs(my_bazooka.geo_vobs-my_sky_grid.Z[h]) <= my_variables.fac_dz*my_sky_grid.dz
            anti_d = ~d
            

            
            hist, xedges, yedges = np.histogram2d((my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.alpha_], 
                                          (my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.Delta_],
                                          bins=[my_sky_grid.nx,my_sky_grid.ny], 
                                          range=(extenti, extentj),
                                          weights=my_bazooka.geo_position_cellsPoints_masse[d])
            #print(np.shape(xedges),np.shape(yedges))
            my_sky_grid.masse_arrays[:,:,h] = list(hist) 
            hist, xedges, yedges = np.histogram2d((my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.alpha_], 
                                          (my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.Delta_],
                                          bins=[my_sky_grid.nx,my_sky_grid.ny], 
                                          range=(extenti, extentj), 
                                          weights=my_bazooka.geo_position_cellsPoints_masse_ism[d])
            my_sky_grid.masse_arrays_ism[:,:, h] = list(hist)
            hist, xedges, yedges = np.histogram2d((my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.alpha_], 
                                          (my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.Delta_],
                                          bins=[my_sky_grid.nx,my_sky_grid.ny], 
                                          range=(extenti, extentj), 
                                          weights=my_bazooka.geo_position_cellsPoints_masse_jet[d])
            #print(np.shape(my_sky_grid.masse_arrays_jet[:, :, h]),np.shape(hist))
            my_sky_grid.masse_arrays_jet[:, :, h] = list(hist)
            ...
            if(my_variables.frame_the_box): 
                hist, xedges, yedges = np.histogram2d((my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.alpha_], 
                                          ...
                                          bins=[my_sky_grid.nx,my_sky_grid.ny], 
                                          range=(extenti, extentj), 
                                          weights=my_bazooka.geo_position_cellsPoints_masse_box_inc[d])
                my_sky_grid.masse_arrays_box_inc[:,:, h] = list(hist)
                
                hist, xedges, yedges = np.histogram2d((my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.alpha_], 
                                          (my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.Delta_],
                                          bins=[my_sky_grid.nx,my_sky_grid.ny], 
                                          range=(extenti, extentj), 
                                          weights=my_bazooka.geo_position_cellsPoints_masse_box_exc[d])
                my_sky_grid.masse_arrays_box_exc[:, :, h] = list(hist)
                        #my_sky_grid.masse_arrays_jet[k, j, h] = np.sum(masse_jet_c[d])
            if(my_variables.floor_ism): 
                hist, xedges, yedges = np.histogram2d((my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.alpha_], 
                                          ...
                                          bins=[my_sky_grid.nx,my_sky_grid.ny], 
                                          range=(extenti, extentj), 
                                          weights=my_bazooka.geo_position_cellsPoints_masse_ism_inc[d])
                #print(">>>>>>>>>>",np.shape(my_sky_grid.masse_arrays_ism_inc[:, :, h]),np.shape(hist))
                my_sky_grid.masse_arrays_ism_inc[:,:, h] = list(hist)
                hist, xedges, yedges = np.histogram2d((my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.alpha_], 
                                          (my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.Delta_],
                                          bins=[my_sky_grid.nx,my_sky_grid.ny], 
                                          range=(extenti, extentj), 
                                          weights=my_bazooka.geo_position_cellsPoints_masse_ism_exc[d])
                my_sky_grid.masse_arrays_ism_exc[:, :, h] = list(hist)
            if(my_variables.floor_jet): 
                hist, xedges, yedges = np.histogram2d((my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.alpha_], 
                                          (my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.Delta_],
                                          bins=[my_sky_grid.nx,my_sky_grid.ny], 
                                          range=(extenti, extentj), 
                                          weights=my_bazooka.geo_position_cellsPoints_masse_jet_inc[d])
                my_sky_grid.masse_arrays_jet_inc[:,:, h] = list(hist)
                hist, xedges, yedges = np.histogram2d((my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.alpha_], 
                                          (my_bazooka.geo_position_Gamma_Delta_alpha[d])[:, my_variables.Delta_],
                                          bins=[my_sky_grid.nx,my_sky_grid.ny], 
                                          range=(extenti, extentj), 
                                          weights=my_bazooka.geo_position_cellsPoints_masse_jet_exc[d])
                my_sky_grid.masse_arrays_jet_exc[:, :, h] = list(hist)
            ...
            my_bazooka.geo_vobs = my_bazooka.geo_vobs[anti_d]
            my_bazooka.geo_position_Gamma_Delta_alpha = my_bazooka.geo_position_Gamma_Delta_alpha[anti_d]
            my_bazooka.geo_position_cellsPoints_masse = my_bazooka.geo_position_cellsPoints_masse[anti_d]
            my_bazooka.geo_position_cellsPoints_masse_ism = my_bazooka.geo_position_cellsPoints_masse_ism[anti_d]
            my_bazooka.geo_position_cellsPoints_masse_jet = my_bazooka.geo_position_cellsPoints_masse_jet[anti_d]
            if(my_variables.frame_the_box): 
                my_bazooka.geo_position_cellsPoints_masse_box_inc = my_bazooka.geo_position_cellsPoints_masse_box_inc[anti_d]
                my_bazooka.geo_position_cellsPoints_masse_box_exc = my_bazooka.geo_position_cellsPoints_masse_box_exc[anti_d]
            if(my_variables.floor_ism): 
                my_bazooka.geo_position_cellsPoints_masse_ism_inc = my_bazooka.geo_position_cellsPoints_masse_ism_inc[anti_d]
                my_bazooka.geo_position_cellsPoints_masse_ism_exc = my_bazooka.geo_position_cellsPoints_masse_ism_exc[anti_d]
            if(my_variables.floor_jet): 
                my_bazooka.geo_position_cellsPoints_masse_jet_inc = my_bazooka.geo_position_cellsPoints_masse_jet_inc[anti_d]
                my_bazooka.geo_position_cellsPoints_masse_jet_exc = my_bazooka.geo_position_cellsPoints_masse_jet_exc[anti_d]

            
            
            print("Progression (en %): {0:.2f} ".format(
                100*(h/my_sky_grid.nz)))
        print("Progression (in %): Finishing... 100 % ")
           
        print("Remaining cells at the end of voxelling : ", np.shape(my_bazooka.geo_vobs))
