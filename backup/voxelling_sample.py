...
rhoISM = np.maximum(read.extract(points2DData, 'tracer_ism', attribute_mode=geo_2D_attribute_mode),0)
rhoJet = np.maximum(read.extract(points2DData, 'tracer_jet', attribute_mode=geo_2D_attribute_mode),0)
...
zedges=[]
for h in range(nz):
    d = np.abs(geo_vobs-Z[h]) <= fac_dz*dz
    #anti_d = ~d
    

    
    hist, xedges, yedges = np.histogram2d((geo_position_Gamma_Delta_alpha[d])[:, alpha_], 
                                  (geo_position_Gamma_Delta_alpha[d])[:, Delta_],
                                  bins=[nx,ny], 
                                  range=(extenti, extentj),
                                  weights=geo_position_cellsPoints_masse[d])
    #print(np.shape(xedges),np.shape(yedges))
    masse_arrays[:,:,h] = list(hist)
    hist, xedges, yedges = np.histogram2d((geo_position_Gamma_Delta_alpha[d])[:, alpha_], 
                                  (geo_position_Gamma_Delta_alpha[d])[:, Delta_],
                                  bins=[nx,ny], 
                                  range=(extenti, extentj), 
                                  weights=geo_position_cellsPoints_masse_ism[d])
    masse_arrays_ism[:,:, h] = list(hist)
    hist, xedges, yedges = np.histogram2d((geo_position_Gamma_Delta_alpha[d])[:, alpha_], 
                                  (geo_position_Gamma_Delta_alpha[d])[:, Delta_],
                                  bins=[nx,ny], 
                                  range=(extenti, extentj), 
                                  weights=geo_position_cellsPoints_masse_jet[d])
    masse_arrays_jet[:, :, h] = list(hist)
    #masse_arrays[k, j, h] = np.sum(masse_c[d])
    #masse_arrays_ism[k, j, h] = np.sum(masse_ism_c[d])
    #masse_arrays_jet[k, j, h] = np.sum(masse_jet_c[d])
    if(frame_the_box): 
        hist, xedges, yedges = np.histogram2d((geo_position_Gamma_Delta_alpha[d])[:, alpha_], 
                                  ...
                                  bins=[nx,ny], 
                                  range=(extenti, extentj), 
                                  weights=geo_position_cellsPoints_masse_box_inc[d])
        masse_arrays_box_inc[:,:, h] = list(hist)
        hist, xedges, yedges = np.histogram2d((geo_position_Gamma_Delta_alpha[d])[:, alpha_], 
                                  (geo_position_Gamma_Delta_alpha[d])[:, Delta_],
                                  bins=[nx,ny], 
                                  range=(extenti, extentj), 
                                  weights=geo_position_cellsPoints_masse_box_exc[d])
        masse_arrays_box_exc[:, :, h] = list(hist)
        #masse_arrays_box_inc[k, j, h] = np.sum(masse_box_inc_c[d])
        #masse_arrays_box_exc[k, j, h] = np.sum(masse_box_exc_c[d])
    #geo_vobs = geo_vobs[anti_d]
    #geo_position_Gamma_Delta_alpha = geo_position_Gamma_Delta_alpha[anti_d]
    #geo_position_cellsPoints_masse = geo_position_cellsPoints_masse[anti_d]
    #geo_position_cellsPoints_masse_ism = geo_position_cellsPoints_masse_ism[anti_d]
    #geo_position_cellsPoints_masse_jet = geo_position_cellsPoints_masse_jet[anti_d]
    #if(frame_the_box): 
    #    geo_position_cellsPoints_masse_box_inc = geo_position_cellsPoints_masse_box_inc[anti_d]
    #    geo_position_cellsPoints_masse_box_exc = geo_position_cellsPoints_masse_box_exc[anti_d]
    #if(voxelling_mode == 'Voxel'):
...
print("Remaining cells at the end of voxelling : ", np.shape(geo_vobs))
